...
type Action =
  ...
  | { type: 'submit'; inputs: Inputs }
  | { type: 'togglePassword' }
  | { type: 'setError'; error: string | null }
  | { type: 'setRedirect' }
  | {
      type: 'updatePasswordCriteria'
...
export function Signup() {
  ...
  if (state.type === 'redirect') {
    return (
      <Navigate
        to={location.state?.source || '/login'}
        replace={true}
      />
    )
  }
  ...
  return (
    <div className="auth-container">
      <h1 className="auth-title">PokerDice Sign Up</h1>
            ...
            <div>
              ...
              <ul className="auth-criteria-list">
                {Object.entries(state.passwordCriteria).map(([key, value]) => (
                  <li
                    key={key}
                    className={`auth-criteria-item ${
                      value ? 'auth-criteria-success' : 'auth-criteria-error'
                    }`}
                  >
                    {value ? '✓' : '×'}{' '}
                    {key === 'minLength'
            ...
            <button
              ...
              disabled={
                ...
                (state.inputs.password !== state.inputs.confirmPassword ||
                  Object.values(state.passwordCriteria).some((c) => !c))
              }
