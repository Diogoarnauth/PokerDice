...
object PokerDiceScorer {
    ...
    fun score(dice: List<Dice>): Int {
        ...
        return when {
            ...
            else -> 200 + dice.maxOf { it.value } // high card bonus
        }
    }
...
class GameLogic(
    ...
    fun applyRound(game: Game, player: Player): GameResult {
        ...
        val currentRound: Round = if (lastRound == null || lastRound.roundOver){
            // criar nova ronda
            val r = Round(
        ...
        val play = Play(playerId = player.id, dice = diceRolls,
        ...
        val allPlayed = currentRound.plays.map{it.playerId}.distinct().size == game.players.size
